...
admin.post('/clear-all', async (c) => {
  ...
  try {
    ...
    // 重置自增 ID（SQLite）
    await DB.prepare("DELETE FROM sqlite_sequence WHERE name IN ('students', 'packages', 'classes', 'payments', 'teachers', 'courses')").run();
    
    return c.json(success({ 
      message: '所有数据已清空',
      cleared: ['students', 'packages', 'classes', 'payments', 'teachers', 'courses']
    }));
  } catch (err) {
